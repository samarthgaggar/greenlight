...
export type ApiStatus = "missing" | "detected" | "connected" | "failed";
export type ThemeChoice = "dark" | "light" | "system";

export type AppState = {
  // Location / action / barrier selection
  selectedLocation: LocationRecord;
  selectedAction: ClimateActionId;
  selectedActionLabel: string;
  selectedDeadZoneId: string;
  selectedDeadZone: DeadZone;
  filteredDeadZones: DeadZone[];
  ...
  interventionRanking: InterventionRanking[];
  scenarioNarration: ScenarioNarration | null;
  scenarioNarrationLoading: boolean;
  // AI analysis
  recommendation: RecommendationResponse | null;
  loadingRecommendation: boolean;
  recommendationError: string | null;
  analysisSubmitted: boolean;
  selectedAIModel: AIModelChoice;
  // App state
  demoMode: boolean;
  developerSettingsOpen: boolean;
  aiMode: AiMode;
  apiStatus: ApiStatus;
  hasApiKey: boolean;
  ...
  // Handlers
  handleSelectLocation: (location: LocationRecord) => void;
  handleCustomLocation: (location: LocationRecord) => void;
  handleSelectAction: (action: ClimateActionId) => void;
  handleSelectDeadZone: (id: string) => void;
  handleToggleIntervention: (id: InterventionId) => void;
  setSelectedAIModel: (model: AIModelChoice) => void;
  runScenarioNarration: () => Promise<void>;
  runRecommendation: () => Promise<void>;
  handleLogoClick: () => void;
  handleDemoModeChange: (enabled: boolean) => void;
  handleAiModeChange: (mode: AiMode) => void;
  applyTheme: (choice: ThemeChoice) => void;
  handlePresentationModeChange: (enabled: boolean) => void;
  handleResetDemoFlow: () => void;
  setDeveloperSettingsOpen: (open: boolean) => void;
};
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const initializedInterventionsRef = useRef(false);

  const filteredDeadZones = useMemo(() => {
    const sourceDeadZones = demoMode
      ? deadZones
      : deadZones.filter((dz) => dz.selectedActions.includes(selectedAction));
    ...
    return matches
      .map((dz) => ({
        ...dz,
        lat: Number((dz.lat + latOffset).toFixed(6)),
        lng: Number((dz.lng + lngOffset).toFixed(6))
      }))
      .sort((a, b) => b.score - a.score);
  }, [demoMode, selectedAction, selectedLocation.lat, selectedLocation.lng]);

  const selectedDeadZone =
    filteredDeadZones.find((dz) => dz.id === selectedDeadZoneId) ?? filteredDeadZones[0];
  const selectedActionLabel =
    climateActions.find((a) => a.id === selectedAction)?.label ?? selectedAction;
  const selectedObservations = useMemo(
    () => observations.filter((o) => o.deadZoneId === selectedDeadZone.id),
    [selectedDeadZone.id]
  );
  ...
  useEffect(() => {
    ...
    function syncDemoState() {
      ...
      if (active) {
        const demoLocation = locations.find((l) => l.name === defaults.location) ?? locations[0];
    ...
    if (isDemoModeFromUrl()) enableDemoMode();
    syncDemoState();
    window.addEventListener("greenlight-demo-mode-change", syncDemoState);
    return () => window.removeEventListener("greenlight-demo-mode-change", syncDemoState);
  }, []);
  ...
  useEffect(() => {
    ...
    function onKeyDown(event: KeyboardEvent) {
      ...
      if (shiftActive && /^[a-z]$/.test(key)) {
        const now = Date.now();
        if (now - shiftedSequenceTimeRef.current > 1500) shiftedSequenceRef.current = "";
  ...
  // Keep selectedDeadZoneId valid when filtered list changes
  useEffect(() => {
    if (!filteredDeadZones.some((dz) => dz.id === selectedDeadZoneId)) {
      setSelectedDeadZoneId(filteredDeadZones[0].id);
    }
  }, [filteredDeadZones, selectedDeadZoneId]);

  // Pre-select top intervention on first load
  useEffect(() => {
    if (initializedInterventionsRef.current || selectedInterventionIds.length > 0) return;
    const top = interventionRanking[0];
    if (top) {
      setSelectedInterventionIds([top.interventionId]);
      initializedInterventionsRef.current = true;
    }
  }, [interventionRanking, selectedInterventionIds.length]);

  // Reset AI results when key inputs change
  useEffect(() => {
  ...
  const handleCustomLocation = useCallback((location: LocationRecord) => {
    ...
    setSelectedLocation({
      ...
      description:
        ...
        "Custom place selected by the user. Greenlight is using local barrier analysis for this MVP."
    });
  }, []);
  ...
  function buildLocalScenarioNarration(): ScenarioNarration {
    const labels = getInterventions(selectedInterventionIds).map((i) => i.label);
    ...
    return {
      ...
      whyItHelps:
        "These improvements target the specific access, safety, and behavior factors that make this barrier score high, so easing them lowers the projected barrier score.",
      adoptionRationale:
        "Projected adoption reflects how visible, safe, and convenient each improvement makes the climate-friendly choice, scaled by local fix feasibility.",
      uncertainty:
        "Projections use synthetic coefficients and local context. Real outcomes depend on verification, maintenance, and community response.",
  ...
  async function runScenarioNarration() {
    ...
    try {
      ...
      if (requestId !== scenarioRequestId.current) return;
      if (!res.ok) throw new Error(`Scenario route returned ${res.status}`);
  ...
  async function runRecommendation() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`Recommendation route returned ${res.status}`);
      const data = (await res.json()) as RecommendationResponse;
      if (requestId !== recommendationRequestId.current) return;
      ...
      setRecommendation({
        ...
        observedEvidence: selectedDeadZone.observedEvidence,
        aiInference:
          "Use the deterministic score and local verification checklist until the API route is available.",
        uncertainty:
          "Greenlight completed this analysis with its reliability layer. Human verification is still required.",
